...
from mmseg.models.losses import gradient_penalty_loss
...
class EncoderDecoderwithSAMPromptSTAdv_Wgangp(BaseSegmentor):
    ...
    def train_step(self, data: Union[dict, tuple, list],
        ...
        ## added by LYU: 2025/04/11
        loss_adv_aux, log_vars_aux = self._get_aux_loss(Pfb_s_dis, Pfb_t_dis, 'dis_aux', self.discriminator_P)
        ...
        loss_adv_dspe = loss_adv_ds + loss_adv_aux
        loss_adv_dspe.backward()
    ...
    ## added by LYU: 2025/04/11
    def _get_aux_loss(self, real_data, fake_data, domain, discriminator):
        losses = dict()
        losses[f'loss_aux_{domain}'] = gradient_penalty_loss(discriminator, real_data, fake_data, 0.01)
        loss_aux, log_vars_aux = self.parse_losses(losses)
        return loss_aux, log_vars_aux
